...
- (UIImage *)popin_applyBlurWithRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage;

@end

@implementation UIViewController (PopinView)
...
- (void)presentPopinController:(UIViewController *)popinController fromRect:(CGRect)rect animated:(BOOL)animated completion:(void(^)(void))completion {
    ...
    if (self.presentedPopinViewController == nil) {
        ...
        if (options & BKTPopinDimmingViewStyleNone) {
            ...
            bgImage = [bgImage popin_applyBlurWithRadius:parameters.radius tintColor:parameters.tintColor saturationDeltaFactor:parameters.saturationDeltaFactor maskImage:nil];
...
- (void)dismissCurrentPopinControllerAnimated:(BOOL)animated completion:(void(^)(void))completion {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RefreshSearchBarNotification" object:nil];
    if (YES == animated) {
        ...
        if ([presentedPopin popinTransitionUsesDynamics]) {
            ...
            [UIView animateWithDuration:0.3f
                                  delay:0.0f options:UIViewAnimationOptionCurveEaseIn
...
- (CGRect)computePopinFrame:(UIViewController *)popinViewController inRect:(CGRect)containerRect {
    ...
    if (NO == CGRectEqualToRect(preferedContainerRect, CGRectUnion(preferedContainerRect, popinPreferedFrame))) {
        if (CGRectGetHeight(popinPreferedFrame) >= CGRectGetHeight(preferedContainerRect)) {
            popinPreferedFrame.size.height = CGRectGetHeight(preferedContainerRect) - kMaryPopinStandardMargin * 2;
        }
        if (CGRectGetWidth(popinPreferedFrame) >= CGRectGetWidth(preferedContainerRect)) {
            popinPreferedFrame.size.width = CGRectGetWidth(preferedContainerRect) - kMaryPopinStandardMargin * 2;
        }
    }
...
- (CGRect)animationFrameForPopinController:(UIViewController *)popinController margin:(CGFloat)margin {
    ...
    if (direction == BKTPopinTransitionDirectionTop || direction == BKTPopinTransitionDirectionBottom) {
        CGFloat multiplier = (direction == BKTPopinTransitionDirectionBottom) ? 1.0f : -1.0f;
        ...
        CGFloat multiplier = (direction == BKTPopinTransitionDirectionRight) ? 1.0f : -1.0f;
        CGFloat popinPosition = 0.0f;
